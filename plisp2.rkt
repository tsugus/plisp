...
(define plisp
  '(
    ...
    ; (error
    ;  . (lambda (code form)
    ;      (Print an error code 'code' and a S-expression 'form'.)))
    (assoc
     . (lambda (key lst)
         (cond ((null lst) (cond ((error '1 key) '())))
    ...
    (apply*
     . (lambda (func args env)
         (cond
           ...
           (else (cond ((error '2 (cons func args)) '()))))))
    (eval
     . (lambda (form env)
         (cond
           ((eq form 't) 't)
...
(define (rev-append x y)
  (cond ((null? x) y)
        (else (rev-append (cdr x) (cons (car x) y)))))
...
(define (error code form)
  (cond ((eqv? code 1) (display "Not found"))
        ((eqv? code 2) (display "Invalid form"))
        (else (display "Error")))
  (display ": ")
  (display form)
  (newline))
  ...
  (cond
    ((and (atom? func) (not (null? func)))
     (cond
       ...
       ((eq? func 'cons) (cons (car args) (cadr args)))
       ((eq? func 'cond) (evcond args env))
       ((eq? func 'apply) (apply_ (car args) (cadr args) env))
       ((eq? func 'function) (list 'funarg (car args) env))
       ((eq? func 'funarg) (cons func args))
       ((eq? func 'error) (error (car args) (cadr args)))
       (else (eval_ (cons (assoc_ func env) args) env))))
    ((eq? (car func) 'funarg)
     (apply_ (cadr func) args (caddr func)))
    ((eq? (car func) 'lambda)
     (eval_ (caddr func)
            (append (assoclist (cadr func) args) env)))
    (else (cond ((error '2 (cons func args)) '())))))
